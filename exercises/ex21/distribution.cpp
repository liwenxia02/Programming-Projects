...
void make_cumulative(std::vector<double> &dist); //call by reference
...
int main( void )
{
  ...
  for( size_t i=0 ; i<sample_num ; i++ ) distribution[ binomial_distribution( generator ) ] += 1./sample_num;
...
void make_cumulative(std::vector<double> &dist){
  for(size_t i = 1; i < dist.size(); i++) dist[i]+=dist[i-1];
}

std::vector< double >::const_iterator naive_find_last_iterator( std::vector< double >::const_iterator begin , std::vector< double >::const_iterator end , double v ){
  std::vector<double>::const_iterator iter = begin;
  for( ; iter!=end; iter++){
    if(*iter<=v && (iter+1==end || *(iter+1)>v) ) return iter;
  }
  return iter;
}
